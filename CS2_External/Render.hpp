...
extern Vector3 GetPositionScreen(int fov, Vector2 AimPunchAngle);
namespace Render
{
	...
	void DrawCrossHair(const CEntity& LocalEntity)
	{ 
		Vector3 point = GetPositionScreen(LocalEntity.Pawn.Fov, { LocalEntity.Pawn.AimPunchAngle.x,LocalEntity.Pawn.AimPunchAngle.y });
		Vec2 SightPos{point.x,point.y};
		Gui.Line({ SightPos.x - MenuConfig::CrossHairSize,SightPos.y }, { SightPos.x + MenuConfig::CrossHairSize,SightPos.y }, MenuConfig::CrossHairColor, 2);
		Gui.Line({ SightPos.x,SightPos.y - MenuConfig::CrossHairSize }, { SightPos.x ,SightPos.y + MenuConfig::CrossHairSize }, MenuConfig::CrossHairColor, 2);
